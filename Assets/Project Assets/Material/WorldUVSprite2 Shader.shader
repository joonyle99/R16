    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (Varyings IN) : SV_Target
            {
                ...
                // 가로(U)만 미러 타일링 → 세로 seam 제거, 세로(V)는 그대로 repeat
                // 아트가 가로로 seamless가 아니어도 경계에서 같은 픽셀을 공유하므로 이음새가 사라짐
                float u     = frac(patternUV.x * 0.5);     // 2타일 주기
                patternUV.x = 1.0 - abs(2.0 * u - 1.0);    // 0→1→0 삼각파
